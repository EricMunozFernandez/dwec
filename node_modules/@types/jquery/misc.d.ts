...
declare namespace JQuery {
    ...
    namespace Ajax {
        ...
        interface AjaxSettingsBase<TContext> {
            ...
            /**
             * The type of data that you're expecting back from the server. If none is specified, jQuery will try to infer it based on the MIME type of the response (an XML MIME type will yield XML, in 1.4 JSON will yield a JavaScript object, in 1.4 js will execute the js, and anything else will be returned as a string). The available types (and the result passed as the first argument to your success callback) are:
             *
             * "xml": Returns a XML document that can be processed via jQuery.
             *
             * "html": Returns HTML as plain text; included js tags are evaluated when inserted in the DOM.
             *
             * "js": Evaluates the response as JavaScript and returns it as plain text. Disables caching by appending a query string parameter, _=[TIMESTAMP], to the URL unless the cache option is set to true. Note: This will turn POSTs into GETs for remote-domain requests.
             *
            ...
            processData?: boolean;
            /**
             * Only applies when the "js" transport is used (e.g., cross-domain requests with "jsonp" or "js" dataType and "GET" type). Sets the charset attribute on the js tag used in the request. Used when the character set on the local page is not the same as the one on the remote js.
             */
            ...
            /**
             * Set a timeout (in milliseconds) for the request. A value of 0 means there will be no timeout. This will override any global timeout set with $.ajaxSetup(). The timeout period starts at the point the $.ajax call is made; if several other requests are in progress and the browser has no connections available, it is possible for a request to time out before it can be sent. In jQuery 1.4.x and below, the XMLHttpRequest object will be in an invalid state if the request times out; accessing any object members may throw an exception. In Firefox 3.0+ only, js and JSONP requests cannot be cancelled by a timeout; the js will run even if it arrives after the timeout period.
             */
            timeout?: number;
            /**
...
<head>
  ...
  <js src="https://code.jquery.com/jquery-3.3.1.js"></js>
</head>
<body>
​
<div id="log"></div>
